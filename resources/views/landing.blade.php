...
<option value="zu">Zulu</option>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                <div class="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-8 hover:shadow-xl transition-shadow">
                    ...
                    <div class="mb-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
                        <p class="text-xs text-blue-800 mb-1 font-semibold">Try Certificate Verification:</p>
                        <a href="https://culturaltranslate.com/verify/CT-UPDATED-94A959F8" target="_blank" class="text-xs text-blue-600 hover:text-blue-700 underline break-all">
                            https://culturaltranslate.com/verify/CT-UPDATED-94A959F8
                        </a>
                    </div>
    ...
    <script>
        ...
        async function translateDemo() {
            ...
            const resultDiv = document.getElementById('demoTranslatedText');
            ...
            try {
                ...
                // Handle both response formats (data.translation and data.data.translated_text)
                const translation = data.translation || data.data?.translated_text;
                const ctsScore = data.cts_score || data.data?.cts_score;
                const sourceLangName = data.source_language_name || data.data?.source_language_name;
                const targetLangName = data.target_language_name || data.data?.target_language_name;

                if (data.success && translation) {
                    // Success - show translation with details
                    const scorePercent = ctsScore ? (parseFloat(String(ctsScore).replace(',', '.')) * 100).toFixed(0) : '95';
                    const isRtl = ['ar', 'he', 'fa', 'ur', 'ar-sa', 'ar-eg', 'ar-ae'].includes(targetLang);
                    const safeTranslation = translation.replace(/'/g, "&#39;").replace(/"/g, "&quot;");
                    
                    resultDiv.innerHTML = `
                        <div class="space-y-4">
                            <p class="text-lg leading-relaxed ${isRtl ? 'text-right' : ''}" dir="${isRtl ? 'rtl' : 'ltr'}">${translation}</p>
                            <div class="flex items-center justify-between pt-4 border-t border-gray-100">
                                <div class="flex items-center gap-4 text-sm text-gray-500">
                                    <span class="flex items-center gap-1">
                                        ...
                                        CTS Score: ${scorePercent}%
                                    </span>
                                    <span>${sourceLangName || sourceLang} → ${targetLangName || targetLang}</span>
                                </div>
                                <button onclick="copyToClipboard(this, '${safeTranslation}')" class="text-indigo-600 hover:text-indigo-700 text-sm font-medium flex items-center gap-1">
