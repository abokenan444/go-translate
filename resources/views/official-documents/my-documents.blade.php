    ...
    <div class="container mx-auto px-4">
        <div class="max-w-6xl mx-auto">
            ...
            <div class="mb-8 flex justify-between items-center">
                ...
                @auth
                <a href="{{ route('dashboard') }}" class="inline-flex items-center gap-2 bg-blue-600 text-white px-6 py-3 rounded-lg font-semibold hover:bg-blue-700 transition shadow">
            ...
            @if(!auth()->check() && $documents->where('status', 'processing')->count() > 0)
                <div class="bg-yellow-50 border-l-4 border-yellow-500 text-yellow-800 p-6 mb-6 rounded shadow-lg">
                    <div class="flex items-start">
                        ...
                        <div class="flex-1">
                            <h3 class="font-bold text-lg mb-2">⚠️ Important: Stay on This Page!</h3>
                            <p class="mb-2">
                                <strong>Your document is being processed right now.</strong> Since you are not logged in, 
                                <span class="underline font-semibold">you will not be able to track your order if you leave this page.</span>
                            </p>
                            <p class="text-sm">
                                Please wait here until your document is ready for download. This usually takes 1-3 minutes.
                                The page will automatically refresh when your document is ready.
                            </p>
                            <div class="mt-4 pt-4 border-t border-yellow-400">
                                <h4 class="font-bold text-md mb-2">Note for Guest Users:</h4>
                                <p class="text-sm">
                                    Translations for guest users do not include the sworn translator seal. To get a translation with a sworn translator seal for official use, please <a href="{{ route("register") }}" class="font-bold underline hover:text-yellow-900">create an account</a> and use this feature from your dashboard.
                                </p>
                            </div>
                        </div>
                    </div>
...
@push('scripts')
@if($documents->where('status', 'processing')->count() > 0 || $documents->where('status', 'paid')->count() > 0)
<script>
    // Auto-refresh page for users with processing or paid documents
    @if(!auth()->check())
        // Guest users: refresh every 10 seconds (they need to stay on page)
        var refreshDelay = 10000;
        var maxRefreshTime = 600000; // 10 minutes
    @else
        // Registered users: refresh every 15 seconds (more relaxed)
        var refreshDelay = 15000;
        var maxRefreshTime = 900000; // 15 minutes
    @endif
    
    let refreshInterval = setInterval(function() {
        console.log('Checking for document completion...');
        window.location.reload();
    }, refreshDelay);
    
    // Stop refreshing after max time (in case something went wrong)
    setTimeout(function() {
        clearInterval(refreshInterval);
        console.log('Auto-refresh stopped after maximum time');
    }, maxRefreshTime);
    
    // Also stop refreshing when user leaves the page
