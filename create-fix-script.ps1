# PowerShell script to apply fixes using direct file content
$server = "culturaltranslate.com"
$user = "root"
$password = "Shatha-Yasser-1992"

Write-Host "=== Connecting to server and applying fixes ===" -ForegroundColor Green
...
# Save script to file
$scriptContent | Out-File -FilePath ".\server-fixes.sh" -Encoding ASCII -NoNewline

Write-Host "`nScript created: server-fixes.sh" -ForegroundColor Yellow
Write-Host "`nTo apply fixes, run these commands in separate terminal:" -ForegroundColor Cyan
Write-Host "scp server-fixes.sh root@culturaltranslate.com:/tmp/" -ForegroundColor White
Write-Host "ssh root@culturaltranslate.com" -ForegroundColor White
Write-Host "bash /tmp/server-fixes.sh" -ForegroundColor White
Write-Host "`nPassword: $password" -ForegroundColor Yellow
